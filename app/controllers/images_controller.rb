class ImagesController < ApplicationController
  ...
  def show
    ...
    @rating = Rating.where(image_id: @image.id, user_id: current_user.id).first unless @rating
    @rating = Rating.create(image_id: @image.id, user_id: current_user.id, score: 0)
  end
  ...
  def statistics
    ...
    @visits_chart = chart_visits(@image.visits_by_day, options)
    @country_chart = chart_countries(@image.visits_by_country, options)
    @age_chart = chart_ages(@image.visits_by_age, options)
  end
